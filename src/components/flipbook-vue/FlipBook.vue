...
export default {
  ...
  computed: {
    ...
    canFlipLeft() {
      return this.forwardDirection === 'left'
        ? this.canGoForward
        : this.canGoBack;
    },
    canFlipRight() {
      return this.forwardDirection === 'right'
        ? this.canGoForward
        : this.canGoBack;
    },
    canZoomIn() {
  ...
  methods: {
    ...
    makePolygonArray(face) {
      if (!this.flip.direction)
        return { polygons: [], minX: Infinity, maxX: -Infinity };
      ...
      const image =
        face === 'front' ? this.flip.frontImage : this.flip.backImage;
    ...
    preloadImages(hiRes = false) {
      ...
      if (hiRes) {
        for (
          ...
          i < this.currentPage + this.displayedPages;
          i++
        ) {
